...
import { TimeOption, canChangeFlexiFunc, getTimeOptions } from '@shared/util_funcs';
...
export class EmployeePostingEditComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.form.get('wef')!.valueChanges.subscribe(x => {
      this.timeOptions = this.timeOptions = getTimeOptions(x,
        this.data.emp.flexi_minutes_current,
        this.data.emp.time_group, this.data.officeTimes);
    ...
    const {canChangeFlexi, lastChangedtext} = canChangeFlexiFunc(
      this.data.emp.flexi_time_wef_current);
